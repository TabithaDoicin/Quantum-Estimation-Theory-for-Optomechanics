...
def prep_qfi_efficient_test(wm, gm, k, d0, n, ep, g0_list, g2_list):
    r_array = np.empty([len(g2_list),len(g0_list)],np.object)
    cov_array = np.empty([len(g2_list),len(g0_list)],np.object)
    alpha_squared_array = np.empty([len(g2_list),len(g0_list)],np.float64)
    for l in range(len(g2_list)):
        for m in range(len(g0_list)):
            r_array[l,m], cov_array[l,m] = main_script.efficient_solver(wm, gm, k, d0, g0_list[m], ep, g2_list[l], n)
            alpha_squared_array[l,m] = 0.5*(np.real(r_array[l,m][2])**2 + np.real(r_array[l,m][3])**2)
